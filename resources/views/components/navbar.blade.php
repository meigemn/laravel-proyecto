        ...
        @auth
            <div class="flex items-center space-x-4 mb-4 ml-4 group rounded-lg p-2">
                <a href="{{ route('profile') }}"
                    class="flex items-center space-x-2 hover:bg-gray-100 hover:text-[#15202b] text-white rounded-lg transition-all">
        ...
        <div class="py-3 flex gap-1 shadow-xl rounded-md">
            ...
            <a href="{{ route('home') }}">
                <div class="group relative px-4 cursor-pointer">
                    ...
                    <span
                        class="absolute -top-8 left-[50%] -translate-x-[50%] z-20 origin-left scale-0 rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium shadow-md transition-all duration-300 ease-in-out group-hover:scale-100 text-white">
                        Inicio
                    </span>
                </div>
            </a>
            ...
            <div x-data="{ showForm: false }" class="fixed bottom-8 right-8 z-50">
                ...
                <button @click="showForm = true"
                    class="bg-blue-500 text-white p-4 rounded-full shadow-lg hover:bg-blue-600 transition-transform transform hover:scale-110">
                ...
                <div x-show="showForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4"
                    @click.away="showForm = false">
                    <div class="bg-[#192734] rounded-xl p-6 w-full max-w-xl">
                        <form action="{{ route('tweets.store') }}" method="POST">
                            @csrf
                            <textarea name="content"
                                class="w-full p-4 bg-transparent text-white placeholder-gray-500 rounded-t-xl focus:outline-none" rows="4"
                                placeholder="¿Qué está pasando?" autofocus></textarea>
            ...
            <div x-data="{ showFollowing: false }" class="group relative px-4 cursor-pointer">
                <div @click="showFollowing = !showFollowing"
                    class="flex h-10 w-10 items-center justify-center rounded-full hover:text-blue-500">
                ...
                <!-- Lista de seguidos -->
                <div x-show="showFollowing" @click.away="showFollowing = false"
                    ...
                    <div class="p-4">
                        ...
                        @forelse(Auth::user()->follows as $user)
                            <a href="{{ route('profile', $user) }}"
                                class="flex items-center space-x-3 p-2 hover:bg-[#22303c] rounded-lg transition-colors">
                ...
                <div onclick="if(confirm('¿Seguro que quieres cerrar sesión?')) { document.getElementById('logout-form').submit() }"
                    ...
                    <div class="flex h-10 w-10 items-center justify-center rounded-full hover:bg-gray-100">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
                            class="h-6 w-6 transition-transform duration-300 hover:scale-110"
                    ...
                    <span
                        class="absolute -top-8 left-1/2 -translate-x-1/2 z-20 origin-center scale-0 rounded bg-gray-800 px-2 py-1 text-xs font-medium text-white shadow-lg transition-all duration-200 group-hover:scale-100">
                        Cerrar sesión
                    </span>
                </div>
            </div>
